...
**春秋检测** 目前99.9%游戏/软件使用的安全检测工具，用来判断你的设备是否处于"非正常环境"——简单说就是查你有没有 root、有没有装模块、有没有开挂。

如果你 root 了设备但想过游戏检测正常游玩，就需要让软件"误以为"你的设备是干净的。
...
| **Root 管理器** | 管理你 root 权限的工具，常见的有 Magisk、KernelSU（KSU）、APatch |
| **KSU 系** | KernelSU 及其变种（LKM 和 GKI 两种模式） |
| **LSPosed / LSP** | 一个 Hook 框架，让你能装各种模块来修改系统行为 |
| **Zygisk Next** | Zygisk 的增强版（部分 root 管理器不自带 Zygisk，需要单独装） |
| **HMA / hma-oss** | 「隐藏应用列表」，用来对指定应用隐藏其他应用/模块 |
| **SUSFS** | 内核级文件/挂载/信息隐藏/伪装工具 |
| **TEESimulatorRS** | 伪装Tee/BL锁状态 |
| **元模块** | 一种挂载的实现方式 |
| **keybox.xml** | 密钥文件，Tricky Store 用它来伪装设备证书 |
| **格机** | 某些恶意可执行脚本(sh/elf)和模块，执行他们会清空你的分区表和userdata，导致不可逆的数据丢失与无法开机完全黑屏 |
...
1. **先别给春秋检测 Root 权限**（见第 23 项）
2. **清除春秋检测应用数据**（见第 6 项）
3. **重启手机，等 1~2 分钟再测**（见第 9、16 项）
4. **关闭 USB 调试**（见第 5 项）
5. **检查 `/sdcard` 下有没有带 `sh` 的文件/文件夹**（见第 10 项）
6. **装上 HMA，配置白名单**（见第 2、8、65 项）
7. **检查 `/data/local/tmp` 的权限和所有者**（见第 40~43、48 项）
8. **更新你的 root 管理器 + Zygisk Next + LSPosed 到最新版**（见第 29、30、37 项）
9. **排查可疑模块**（见第 21、51 项）
10. **检查 TEE 相关**（见第 14、18、68~77 项）

> 💡 每一步改完后**重启手机**再测，不要一次改太多，否则不知道哪个操作生效了。

---

### Q4：KSU 的 LKM 和 GKI 有什么区别？

- **GKI（Generic Kernel Image）：** 直接替换内核镜像，兼容性好但门槛高，费电，隐藏性理论最强，适合系统深度搞机的设备，理论上对内核版本无要求。
- **LKM（Loadable Kernel Module）：** 以内核模块形式加载，更灵活，推荐大多数用户使用，需要内核版本大于等于5.10。
- **并没有哪个隐藏能力更好这一说，无需盲目追求某种工作模式**
...
这是 Zygisk Next 的一个选项，作用是：对于排除列表中的应用，Zygisk Next 会把挂载改动"还原"掉，让应用看不到模块的存在。

设置路径：**Zygisk Next → 排除策略 → 选择「仅还原挂载」**

---

### Q6：我明明没 root，为什么春秋还是报？

可能的原因：
- **小米/红米新系统**（2025 年 3 月后更新）：系统构建时间和安全补丁时间不一致，春秋会误报，无视即可（见第 14 项）
- **设备 Bootloader 解锁了但没 root**：春秋也会检测到解锁状态（见第 72 项）
- **装了一些看似无害但被检测的模块**（如温控模块，见第 51 项）
- **开某些敏感项，不能实锤但高危**（MT管理器？shizuku？）

---

### Q7：更新模块后反而报了一些检测点怎么办？

常见！可以尝试：
- **降级** root 管理器 / Zygisk Next / LSPosed 到之前正常的版本
- 禁用所有模块 → 重启 → 逐个启用 → 逐个测试，定位问题模块
...
不一定。
如今绝大多数软件（99%）都不检测应用列表，因为没有意义。

---

### Q9：春秋检测说「TEE 环境不可信」怎么办？

非小米/红米的高通设备就是过不了。
不要听信掩耳盗铃的方法（隐藏Soter Service路径），只会死得更快。

---

### Q10：我的问题不在文档覆盖范围内怎么办？

- 设备型号，管理器（版本），模块列表发我，我帮你排查。

---

### Q11：什么是「格机」？风险大吗？

**格机** 指恶意格式化设备底层分区，导致设备无法启动（变砖）。

> ⚠️ **安全提醒：**
> - 操作前备份重要数据（一加特别需要备份ocdt分区，很重要，不然一辈子进不去fastboot）
> - 不确定的命令/文件不要乱执行
...
- 过检测 ≠ 开挂，过检测只是隐藏 root，不代表你用了外挂。
- 但如果你**同时**用了修改游戏数据的模块（如透视、自瞄），那封号风险依旧存在。
- 本文档仅讨论**环境隐藏**，不涉及任何游戏修改功能。
- 请注意，游戏检测和检测软件完全不一样。
