...
> 只想过春秋的去hma-oss把春秋的限制zygote权限除了Inet_GID全开。
...
- **解决方法：** 使用 SUSFS / PathMask 隐藏 soterservice，并使用hma-oss对检测隐藏soter service应用。
...
> 只需要把它下载下来执行一下就行。
> 请做好卡开机的心理准备(虽然大概率不会)，如果卡开机尝试在recovery模式下删除/data/cache这个目录。如果你手机里没有什么重要的数据，可以直接在recovery里格式化也可以解决。
> 执行不了的换老release。
...
- **解决方法：** ** 暂时把这条检测项理解为误报 **，hma-oss打开春秋检测的限制zygote权限(除了倒数第二个INET_GID)。
> 一个为了检测是否有应用阻止自己扫盘的检测项居然可以通过高级一点的阻止他扫盘的方法“解决”。
...
- **检测项：** 检测到ro.secureboot.lockstate属性值为unlocked。
- **解决方法：** 执行以下脚本。

```
su
'/data/adb/ksud' resetprop ro.secureboot.lockstate locked
```
...
- **检测项：** 解锁bl后证书从L1变成了L3，但检测到没有返回正确L1证书应该返回的数据，说明是伪造了L1证书。
- **解决方法：** 完全可以不用管，如果想搞的话请使用rkp config这个应用获取rkp远程密钥。
> 但是你假回锁/免解/压根没root也会概率报
> 存在误报可能

---

### 97. Tampered Attention Key（31）
